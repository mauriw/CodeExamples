...
DATA_DIRECTORY = 'data/no_deconv/'
OUT_DIR ='small_aws_output/'
...
def get_model():
    ...
    x = Conv2D(128, 5, strides=1, activation='relu', padding='same', kernel_regularizer=regularizers.l2(0.01))(x)
    x = Conv2D(64, 7, strides=1, activation='relu', padding='same', kernel_regularizer=regularizers.l2(0.01))(x)
    x = Conv2D(1, 11, strides=1, activation='relu', padding='same', kernel_regularizer=regularizers.l2(0.01))(x)
    
    # x = Convolution2D(512, 5, 5, init='normal', activation='relu', border_mode='same', name='conv4')(x)
    # x = Convolution2D(512, 5, 5, init='normal', activation='relu', border_mode='same', name='conv5')(x)
...
def main():
    ...
    if len(sys.argv) != 1 and sys.argv[1] == 'predict':
        ...
        lr_scheduler = LearningRateScheduler(
            schedule=PeriodicLR(LEARNING_RATE, 50, 0.5))
